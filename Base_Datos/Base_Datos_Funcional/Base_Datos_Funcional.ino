...
const int pinSalida = 22; // Número del pin de salida
...
const int humsuelo = 33;    // Número del pin del sensor de humedad del suelo
float ValHum = 0; // Variable para almacenar el valor de la humedad del suelo
float TIM_R_Hum = 0; // variable apra medir el valor en tiempo real del sensor

const int Sensor_h = 32; // Pin del sensor de humedad


WiFiClient espClient; // parametros para Cliente WiFi
...
void loop() {
    ...
    if (now - lastMsg > 120000) {
       ...
       ValHum = map(analogRead(humsuelo), 4095, 0, 0, 100); 
        ...
        Serial.println(" %");
        if (ValHum > 40) {
          digitalWrite(pinSalida, LOW);  // Si x es mayor que 20, apagar la salida
          Serial.println("valvula cerrada");
          valv=0;
        } else {
          digitalWrite(pinSalida, HIGH);  // Si x es menor o igual a 20, encender la salida
